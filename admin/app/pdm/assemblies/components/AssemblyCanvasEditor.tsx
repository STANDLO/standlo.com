...
type AssemblyCanvasEditorProps = {
    parts: AssemblyPartNode[];
    onChangeParts: (parts: AssemblyPartNode[]) => void;
};
...
export default function AssemblyCanvasEditor({ parts, onChangeParts }: AssemblyCanvasEditorProps) {
    ...
    // Effect to aggressively load missing meshes for any part currently in the assembly
    useEffect(() => {
        const loadMissingMeshes = async () => {
            for (const node of parts) {
                ...
                if (partDef && partDef.meshId && !meshCache[partDef.meshId]) {
                    try {
                        const meshData = await OrchestratorClient.read<MeshEntity>("mesh", partDef.meshId);
                        if (meshData) {
                            setMeshCache(prev => ({ ...prev, [partDef.meshId as string]: meshData }));
                        }
