...
export default function CanvasAssembliesAdminPage() {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const assembliesData = await OrchestratorClient.list<AssemblyEntity>("assembly", { limit: 100 });
            const resultData = assembliesData || [];
    ...
    const openEditor = async (item: AssemblyEntity | null) => {
        if (item) {
            if (item.id) {
                setLoading(true);
                try {
                    const details = await OrchestratorClient.getDetails<AssemblyEntity>("assembly", item.id);
                    if (details) {
                        setEditing({
                            ...details,
                            parts: details.parts || [],
                    ...
                    alert("Could not load assembly details.");
                } finally {
                    setLoading(false);
                }
            } else {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            const calculatedCost = (editing.parts || []).reduce((acc, p) => acc + (partDict[p.partId]?.cost || 0), 0) +
                (editing.processes || []).reduce((acc, p: AssemblyProcessNode) => acc + (processDict[p.processId]?.cost || 0) * (p.quantity || 1), 0);

            const calculatedPrice = (editing.parts || []).reduce((acc, p) => acc + (partDict[p.partId]?.price || 0), 0) +
                (editing.processes || []).reduce((acc, p: AssemblyProcessNode) => acc + (processDict[p.processId]?.price || 0) * (p.quantity || 1), 0);
            ...
            if (payload.id) {
                await OrchestratorClient.update("assembly", payload);
            } else {
                await OrchestratorClient.create("assembly", payload);
            }
            setEditing(null);
            loadData();
        } catch (e) {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await OrchestratorClient.delete("assembly", id);
            loadData();
        ...
        <div className="ui-canvas-layout">
            <header className="ui-canvas-header">
                <h1 className="ui-canvas-title">
                    <span className="text-blue-600 dark:text-blue-400 mr-2">Assemblies</span> Manager
                </h1>
                <p className="ui-canvas-description">
                    Manage hierarchical Assemblies containing parts and sub-components.
                </p>
            </header>
            ...
            <div className="ui-canvas-grid">
                <div className="ui-canvas-panel">
                    <div className="ui-canvas-panel-header">
                        <div>
                            <h2 className="ui-canvas-panel-title">Assemblies</h2>
                            <p className="ui-canvas-panel-subtitle">Collection: `assemblies`</p>
                        </div>
                        <button onClick={() => openEditor(null)} className="ui-canvas-btn-primary">
                            <Plus className="w-4 h-4 mr-2" /> Add Assembly
                        </button>
                    </div>
                        ...
                        {editing ? (
                            <form onSubmit={handleSave} className="ui-canvas-form">
                                <div className="ui-canvas-form-header">
                                    <h3 className="ui-canvas-form-title">{editing.id ? "Edit Assembly" : "New Assembly"}</h3>
                                                ...
                                                <div className="flex justify-between items-center mb-4">
                                                    ...
                                                    <select
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            const newProcess: AssemblyProcessNode = { id: crypto.randomUUID(), processId: e.target.value, quantity: 1 };
                                ...
                                <div className="ui-canvas-form-actions">
                                    <button type="submit" className="ui-canvas-btn-success shadow-md px-6">
                                        <Save className="w-4 h-4 mr-2" /> Save Assembly
                                    </button>
                                </div>
                            </form>
                        ) : loading ? (
                            ...
                            <table className="ui-table">
                                ...
                                <tbody className="ui-table-tbody">
                                    {assemblies.map(assembly => (
                                        <tr key={assembly.id} className="ui-table-tr">
                                            ...
                                            <td className="ui-table-td">{assembly.parts?.length || 0}</td>
                                            <td className="ui-table-td">{assembly.cost?.toFixed(2) ?? "0.00"}</td>
                                            <td className="ui-table-td">{assembly.price?.toFixed(2) ?? "0.00"}</td>
                                            <td className="ui-table-td text-right">
                                                <button onClick={() => openEditor(assembly)} className="ui-canvas-btn-icon mr-2">
                                                    <Edit2 className="w-4 h-4" />
                                                </button>
                                                <button onClick={() => assembly.id && handleDelete(assembly.id)} className="ui-canvas-btn-icon-danger">
                                                    <Trash2 className="w-4 h-4" />
                                                </button>
                                            </td>
                                        </tr>
                                    ...
                                    {assemblies.length === 0 && (
                                        <tr>
                                            <td colSpan={6} className="ui-table-empty">
                                                No assemblies found. Create one.
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
