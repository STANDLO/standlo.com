...
function HomeCard({ variant, title, onClick }: { variant: string, title: string, onClick: () => void }) {
  ...
  return (
    <Card 
      ...
      layout="full"
      onClick={onClick}
      className={`ui-home-card ui-home-card-${variant}`}
      icon={false}
    >
      <div className="ui-home-card-header" />
      <div className="ui-home-card-content">
        <span className="ui-home-card-title-text">{title}</span>
      </div>
    </Card>
  );
}

function HomeSync({ orientation }: { orientation: "horizontal" | "vertical" }) {
...
export function HomeOverlay({ active = true }: { active?: boolean }) {
  ...
  return (
    <div className="ui-home absolute inset-0 z-50 pointer-events-auto bg-transparent animate-in fade-in duration-500">
      ...
      <div 
        className="ui-home-exit absolute pl-6 pr-6 pt-6 top-0 right-0 cursor-pointer text-muted-foreground hover:text-slate-400 transition-colors z-[60] bg-black/20 rounded-bl-3xl" 
        onClick={handleClose} 
      ...
      {cards.map((card) => (
        <div 
          key={card.title}
          className={`ui-home-cell ${card.colClass} ${card.rowClass} col-span-2 row-span-2 z-10`}
        >
          <div 
      ...
      {syncsHorizontal.map((sync, i) => (
        <div key={`sh-${i}`} className={`${sync.colClass} ${sync.rowClass} col-span-1 row-span-2 z-0 flex items-center justify-center`}>
          <HomeSync orientation="horizontal" />
        </div>
      ))}
