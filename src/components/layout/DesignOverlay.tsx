...
const StandloCanvas = dynamic(() => import("./design/Design"), { ssr: false });

function DesignOverlayInner({ active = true }: { active?: boolean }) {
...
export function DesignOverlay({ active = true }: { active?: boolean }) {
    const pathname = usePathname();
    ...
    return (
        <div className="absolute inset-0 z-0 pointer-events-auto">
            <Suspense fallback={null}>
                <DesignOverlayInner active={finalActive} />
            </Suspense>
        </div>
    );
}
